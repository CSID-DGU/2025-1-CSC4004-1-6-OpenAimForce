...
def extract_sequences_from_log(log_content_lines, player_ingame_id, max_seq_len=None):
    ...
    for i, line in enumerate(log_content_lines):
            ...
            if pos_match and ang_match and current_t_microsec is not None:
                ...
                if prev_t_microsec is not None and current_t_microsec > prev_t_microsec:
                    ...
                    if prev_omega_yaw is not None: 
                        current_event_features[9] = (omega_y - prev_omega_yaw) / dt
                        current_event_features[10] = (omega_p - prev_omega_pitch) / dt
        ...
        if event_occurred:
            ...
            #     current_event_features[11] = cumulative_hits_given_by_player / cumulative_shots_fired_by_player
            # else:
            #     current_event_features[11] = 0.0
...
for target_hack_type in HACK_TYPES_TO_MODEL:
    ...
    # 1.4. 클래스 가중치 계산
    print(f"\n[Task {target_hack_type}] Step 1.4: Calculating class weights...")
    ...
    model = Sequential([
        ...
        Bidirectional(LSTM(192, return_sequences=True, kernel_regularizer=regularizers.l2(l2_lambda))), 
        Dropout(0.5), 
        Bidirectional(LSTM(128, return_sequences=False, kernel_regularizer=regularizers.l2(l2_lambda))), 
        Dropout(0.5), 
    ...
    opt = tf.keras.optimizers.Adam(learning_rate=0.00007, clipnorm=1.0)
