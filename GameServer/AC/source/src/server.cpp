    ...
    if (cl && !cl->isauthed)
    {
        ...
        { // SV_CONNECT
            ...
            loopv(clients) if (clients[i]->type != ST_EMPTY)
            {
                ...
                {
                    if (!strcmp(clients[i]->name, arg_team1[j])) { in_team1 = true; break; }
                    if (!strcmp(clients[i]->name, arg_team2[j])) { in_team2 = true; break; }
                }
